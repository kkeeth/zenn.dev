...
ん〜なるほど！ランダムで `TWO_PI(2π)` までの値を取得していること，またこちらの式 `Math.sqrt(1 - vz * vz)` なんてまさに __半径１の円の方程式__ にしか見えませんw
...
全く違う表現になりましたので，`p5.Vector.random3D()` メソッドを使うことがポイントのようです．

![](https://storage.googleapis.com/zenn-user-upload/0587dd9d526e-20221106.png)


:::details 余談（`p5.Vector.random2D()` メソッド）
勘の良い方は，今回は結局 2D の絵を描いているのだから `p5.Vector.random2D()` というメソッドを用いれば良くない？と思われたかと思います．そのご指摘はまっこと仰るとおりで実際に私も試してみましたが，どうもこちらでも違った表現になってしまいました．
...
ここの `/ 90` の部分がそれで，ここを削除するとわかりますが，流れていくような表現にはなりません．これは元々の座標位置の値がランダムですので，差分を小さくしないと次の描画位置も見た目上はバラバラになってしまいます．ですので，小さくするために `/ 90` で小さくしていると思われます．
