...
ん〜なるほど！ランダムで `TWO_PI(2π)` までの値を取得していること，またこちらの式 `Math.sqrt(1 - vz * vz)` なんてまさに __原点を中心とした半径１の円の方程式__[^2] の変形にしか見えませんw また `vx, vy` も，これはどう見ても __極座標__[^3] ですのでもろに円ですね😂
...
という関係になっており，結果 __半径 `r` の四角がプロットされる__ ことになります．この増分・変化量をなるべく連続的にしたいがために `/ 90` で小さくしているのだろうというのが，理由の２つ目となりますが，ここは予想の粋を出ません 🙇


## ③ `^` （ビット排他的論理和）

すでにだいぶ長く文章が多くて申し訳ないですが，お次は普段見ることが殆どない（個人の感覚です）演算子である，ビット排他的論理和[^4] を意味する `^` です．電気回路に触れたことがある方は `XOR` には馴染みがあり，分かりやすいかもしれません．

[^4]: ２つの入力が共に真または偽の場合は偽，片方のみが真または偽の場合は真となる論理演算のこと．参考リンク: [ビット排他的論理和（MDN）](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Operators/Bitwise_XOR)

ビット排他的論理和そのものは説明できますが，__私は意図的に使ったことがないのでどういう時に利用するのかちょっと分からず…__ こちらは __どなたかわかる方ご教示いただけますと大変に助かります🙇__
...
もう一つの謎，__なぜ四角が複数個プロットされるのか__ について．上の `r = 20` の例がまさにそれですが，これは②の話の続きで，
...
更に言うと，理解が浅く雑な点も多いため，__間違っていることを書いている可能性は大いにありえます__．こちらについては遠慮なくマサカリをいただけますと大変に助かります 🙇
...
また，明日は [5h5k / Torii](https://adventar.org/users/51235) さんによる『何かこしらえて「作ってみた」って言います』とのことです❗お楽しみに❗
