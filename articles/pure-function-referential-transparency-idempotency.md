...
例 3・例 5（冪等だけど純関数でない）と例 6（純関数だけど冪等でない）が，3 概念が独立していることを示す好例です．
...
一方，`useEffect` では副作用を扱いますが，**effect を何度実行しても安全（再実行に耐える）ように設計する**ことが重要です．React の Strict Mode は開発時に effect をわざと 2 回実行し，この「再実行への耐性」が欠けていないかを検出します．次の例ではクリーンアップ関数で `cancelled` フラグを立て，古いレスポンスで `state` を上書きしてしまう競合状態（race condition）を防いでいます：
