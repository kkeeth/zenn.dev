...
:::message
なお，ここで言う**副作用**とは「外部の状態を書き換える」または「外部の状態を読み取る（I/O など）」操作を指します．後述の冪等性と区別するうえで，この「決定論的である」という性質も覚えておいてください．
:::
...
冪等性は，**同じ操作を何度実行しても，1 回実行した時と同じ結果（状態）になる** という性質です．数式で書くと `f(f(x)) === f(x)` が成り立つイメージですね．

ここが重要なポイントで，冪等性で見るのは **「2 回目以降の操作は，1 回目が作った状態に対して，何の影響も与えない」** という点です．

:::message
**冪等性 ≠ 決定論性** に注意してください．「同じ入力で常に同じ出力を返す（決定論的）」は純関数の条件①であって，冪等性とは別物です．冪等性は「**操作を重ねがけしても，1 回だけ実行したときと結果が同じ**」かどうかを見ます．次の `abs` の例も，単に `abs(-5)` を 3 回呼んで同じ値になること（＝決定論性）ではなく，**`abs(abs(x))` のように適用を重ねても結果が変わらないこと**が冪等性のポイントです．
:::
...
| **冪等性**     | 操作の性質 | 操作を重ねても 1 回実行時と同じ結果（状態）になる |
...
- **冪等性**: ○（`abs(abs(-5))` も `abs(-5)` と同じ `5`．適用を重ねても結果が変わらない）
...
- **冪等性**: ○（外部の状態を一切変えないので，何度実行しても系の状態は 1 回実行時と同じ．GET が冪等なのと同じ理屈）

「実行するたびに違う時刻が返ってくるのに冪等なの？」と引っかかった方は，前述の **冪等性 ≠ 決定論性** を思い出してください．`getCurrentTime` は決定論的ではありませんが，状態には何の影響も与えないので操作としては冪等です．純関数の条件と冪等性の条件が別物であることがよく分かる例だと思います．
...
- **冪等性**: ×（`inc(inc(2))` は `4` で，`inc(2)` の `3` と一致しない）

例 3 とは逆に，こちらは **純関数だけど冪等ではない** ケースです．例 3 と例 6 を見比べると，純関数と冪等性は**どちらか一方だけ成り立つこともある，独立した概念**だと分かりますね 🙌
...
例 3〜5（冪等だけど純関数でない）と例 6（純関数だけど冪等でない）が，3 概念が独立していることを示す好例です．
...
- **冪等性**: システムの安全性と信頼性を高める性質．特に API 設計や分散システムで重要（ネットワーク障害でリクエストをリトライしたり，同じメッセージが重複配信（at-least-once）されたりしても，処理が冪等なら状態が壊れません）
